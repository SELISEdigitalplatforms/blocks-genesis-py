...
def test_tenant_model_fields():
    params = JwtTokenParameters(
        Issuer='issuer', Subject='subject', Audiences=['aud'],
        PublicCertificatePath='path', PublicCertificatePassword='pubpass',
        PrivateCertificatePassword='privpass',
        CertificateStorageType=CertificateStorageType.AZURE,
        CertificateValidForNumberOfDays=10, IssueDate=datetime.now()
    )
    tenant = Tenant(
        _id='id', CreatedAt=datetime.now(), UpdatedAt=datetime.now(),
        TenantId='tid', IsAcceptBlocksTerms=True, IsUseBlocksExclusively=False,
        Name='name', DBName='db', IsDisabled=False, DbConnectionString='conn',
        TenantSalt='salt', JwtTokenParameters=params, IsRootTenant=True,
        Environment='prod', TenantGroupId='grp'
    )
    ...
    assert tenant.environment == 'prod'
    assert tenant.tenant_group_id == 'grp'
    assert tenant.jwt_token_parameters.issuer == 'issuer'
